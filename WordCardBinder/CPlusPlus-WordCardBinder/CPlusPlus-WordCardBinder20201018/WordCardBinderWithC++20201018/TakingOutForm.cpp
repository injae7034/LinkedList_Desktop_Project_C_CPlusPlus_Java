...
BOOL TakingOutForm::OnInitDialog()
{
	...
	//1.6 단어카드철 프레임 윈도우의 단어카드청에 단어카드가 남아있으면
	current = wordCardBinderForm->wordCardBinder->GetCurrent();
	if (current != NULL)
	{
		//1.6.1 단어카드철 프레임 윈도우에 현재 단어카드를 출력한다.
	...
	//1.7 단어카드철 프레임 윈도우의 단어카드청에 남아있는 단어카드가 없으면
	else
	{
		//1.7.1 단어카드철 프레임 윈도우에 공란을 출력한다.
