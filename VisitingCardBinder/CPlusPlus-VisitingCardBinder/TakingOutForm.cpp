...
BOOL TakingOutForm::OnInitDialog()
{
	...
	//1.7 명함철에 명함을 꺼낸 후에 명함이 없으면
	else 
	{
		//1.7.1 명함철 프레임 윈도우에 공란을 출력한다.
...
void TakingOutForm::OnClose()
{
	...
	int messageBoxButton = MessageBox("끼우시겠습니까?", "꺼내기윈도우", MB_YESNOCANCEL);
	//2.2 "예" 버튼을 클릭했을 때
	if (messageBoxButton == IDYES)
	{
		...
		//2.2.6 명함철 프레임 윈도우에 끼운 명함을 출력한다.
