...
int main(int argc, char* argv[])
{
	...
	printf("\n");
	Move(&visitingCardBinder, indexes[0]);
	printf("%s %s %s %s %s %s %s %s %s\n", visitingCardBinder.current->personal.name, 
		visitingCardBinder.current->personal.position, visitingCardBinder.current->personal.cellularPhoneNumber, 
		visitingCardBinder.current->personal.emailAddress, visitingCardBinder.current->company.name, 
		visitingCardBinder.current->company.address, visitingCardBinder.current->company.telephoneNumber,
		visitingCardBinder.current->company.faxNumber, visitingCardBinder.current->company.url);
	printf("\n");
	if (indexes != NULL)
	{
		free(indexes);
